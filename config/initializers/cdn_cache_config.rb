raw_yaml = ERB.new(Rails.root.join('config/cdn_cache.yml').read).result
redis_config = (YAML.safe_load(raw_yaml, aliases: true)[Rails.env] || {}).symbolize_keys

Rails.configuration.cdn_cache = nil
...
if redis_config && redis_config[:enabled]
  ...
  begin
    Rails.configuration.cdn_cache = Redis.new(redis_config)
