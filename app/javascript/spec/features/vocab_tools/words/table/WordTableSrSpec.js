...
import WordTableSr from 'features/vocab_tools/words/table/WordTableSr';
const unitData = {
  id: 1,
  inCourse: true,
  lessons: [
...
const getWrapper = (propsData) => {
  return shallowMount(WordTableSr, {
    propsData,
    stubs: ['AddWordIcon', 'EditWord', 'HeaderRow', 'LessonHeaderRow', 'NewWord'],
  });
};
...
describe('WordTableSr', () => {
  describe('when unit has 2 lessons and 4 words', () => {
    beforeEach(() => {
      wrapper = getWrapper(commmonProps);
    });

    describe('when WordTableSr is mounted', () => {
      it('displays 2 tables one for each lesson', () => {
        expect(wrapper.findAll('.test-table-vocab-tools-words')).toHaveLength(2);
      });
  ...
  describe('when some lessons are not in-course', () => {
    describe('when viewAllLessons is true', () => {
      ...
      describe('when WordTableSr is mounted', () => {
        it('displays all 4 lessons', () => {
          expect(wrapper.findAllComponents({ name: 'LessonHeaderRow' })).toHaveLength(4);
        });
      });
    });
  });
  ...
  describe('when viewAllLessons is false', () => {
    ...
    describe('when WordTableSr is mounted', () => {
      it('displays 2 in-course lessons', () => {
        expect(wrapper.findAllComponents({ name: 'LessonHeaderRow' })).toHaveLength(2);
      });
    });
  });
