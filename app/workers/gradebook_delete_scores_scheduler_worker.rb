...
# jobs to delete scores for them.
class GradebookDeleteScoresSchedulerWorker
  include Sidekiq::Worker
  include WorkerInstrumentation

  def perform(
    end_date_min = (Time.zone.today - 2.years).to_s,
    end_date_max = (Time.zone.today - 2.years).to_s
  )
    logger_data_merge(
    ...
    course_ids.each_with_index do |course_id, i|
      ...
      # deleted.
      GradebookDeleteScoresWorker.perform_in(i * 30, course_id)
    end
  end
end
