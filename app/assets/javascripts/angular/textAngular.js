/*
  Please Note: The textAngular souce has been modified for our purposes.
  - 233: Change functionality of "Clear" to work for Header Tags.
  - 560-562: Trigger update on Accent Bar item click.
  - 724-754: Add Titles to Button Elements
*/
...
(function(){ // encapsulate all variables so they don't become global vars
  ...
  */
  ...
  textAngular.directive("textAngular", [
    '$compile', '$timeout', '$log', 'taOptions', 'taSanitize', 'textAngularManager', '$rootScope', 'taIsFocusSet',
    function($compile, $timeout, $log, taOptions, taSanitize, textAngularManager, $rootScope, taIsFocusSet){
      return {
        require: '?ngModel',
        scope: {},
        restrict: "EA",
        link: function(scope, element, attrs, ngModel){
          ...
          if ( scope.html === null || scope.html === undefined ) {
            scope.html = '';
          }
          ...
          if(attrs.ngModel){
            ...
            ngModel.$render = function(){
              ...
              if ( !taIsFocusSet() ) {
                ...
                $rootScope.$broadcast( 'setFocusIfNeeded', { scope: scope } );
              }
            };
          }else{
  ...
  ]).directive('taBind', ['taSanitize', '$timeout', 'taIsFocusSet', function(taSanitize, $timeout, taIsFocusSet){
    ...
    return {
      ...
      link: function(scope, element, attrs, ngModel){
        ...
        ngModel.$render = function(){
          ...
          if ( document.activeElement !== element[0] && !taIsFocusSet() ) { // i.e. 'if focus is not set'
            // catch model being null or undefined
            ...
            if(_isContentEditable){
              ...
              if ( !taIsFocusSet() ) { // i.e. 'if focus is not set'
                element.val(val);
              }
            }
          }
      ...
      return {
        ...
        link: function(scope, element, attrs){
          ...
          setupToolElement = function(toolDefinition, toolScope){
            ...
            var toolTitle = toolScope.name;
            switch(toolScope.name) {
              case "ul":
                toolTitle = "unordered list";
                break;
              case "ol":
                toolTitle = "ordered list";
                break;
              case "h1":
                toolTitle = "large header";
                break;
              case "h2":
                toolTitle = "small header";
                break;
              case "justifyLeft":
                toolTitle = "align left";
                break;
              case "justifyCenter":
                toolTitle = "align center";
                break;
              case "justifyRight":
                toolTitle = "align right";
                break;
              case "clear":
                toolTitle = "clear formatting";
                break;
            }

            toolElement.attr('title', toolTitle);
  ...
  }]).factory( 'taIsFocusSet', [ function() {
    return function() {
      var jq_elm = $(':focus');
      return ( jq_elm.length && jq_elm.attr('ng-model') );
    };
  }]);
