describe Lti::HomePageDeepLinkJwt do
  ...
  describe '#to_h' do
    ...
    def result
      jwt = described_class.new(launch.guid).to_h[:jwt]
      JWT.decode(jwt, public_key, true, algorithm: 'RS256').first
    end

    it 'logs an lti event' do
      described_class.new(launch.guid).to_h
    ...
    it 'has a content item that includes the homepage view param' do
