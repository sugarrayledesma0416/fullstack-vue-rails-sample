describe Lti::ActivityDeepLinkJwt do
  ...
  describe '#to_h' do
    ...
    def result
      hash = described_class.new(activity.id, launch.guid).to_h
      JWT.decode(hash[:jwt], public_key, true, algorithm: 'RS256').first
    end

    it 'logs an lti event' do
      described_class.new(activity.id, launch.guid).to_h
    ...
    it 'has a content item that includes the activity content information' do
      title = "#{lesson.display_name} - #{strand.display_name} - #{activity.title}"
    ...
    it 'returns a link_info key with the activity content information' do
      hash = described_class.new(activity.id, launch.guid).to_h

      expect(hash[:link_info]).to eq(
        "#{lesson.display_name} - #{strand.display_name} - #{activity.title}"
      )
    end

    it 'removes html entities and tags from content item title' do
      strand = build_stubbed(:toc_entry, title: '<b>A&amp;B</b>')
      lesson = create(
        :lesson,
        name: '<i>C&gt;D</i>',
        toc_entries: [strand],
      ...
      result = JWT.decode(hash[:jwt], public_key, true, algorithm: 'RS256').first

      expected_value = 'C>D - A&B - G<H'
