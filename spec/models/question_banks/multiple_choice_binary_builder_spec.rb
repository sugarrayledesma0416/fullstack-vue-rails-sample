describe QuestionBanks::MultipleChoiceBinaryBuilder do
  let(:options) { %w[a b] }
  ...
  describe '#content_object' do
    ...
    it 'calls .from_csv_hash on the MultipleChoiceTrueFalseContent class, ' \
  ...
  describe '#valid?' do
    ...
    context 'when the "correct_answer" header is missing,' do
      before do
        valid_row.delete(:correct_answer)
      end
