describe AI::OverallComment do
  describe 'validations' do
    ...
    context 'when creating a new record,' do
      ...
      it 'is valid if the prompt is not blank' do
        entry = build(:ai_overall_comment, prompt:)

        expect(entry).to be_valid
      end

      it 'is invalid if the prompt is blank' do
        entry = build(:ai_overall_comment, prompt: nil)
    ...
    context 'when updating an exiting record,' do
      let(:entry) { create(:ai_overall_comment) }
      let(:prompt) { create(:ai_overall_comment_prompt) }
    ...
    context 'when the entry has not been reviewed by anyone' do
      let(:entry) { build(:ai_overall_comment, reviewed_by: nil) }
    ...
    context 'when the entry has been reviewed by someone' do
      ...
      let(:entry) { build(:ai_overall_comment, reviewed_by: user) }
    ...
    context 'when the entry has not been rated' do
      let(:entry) { build(:ai_overall_comment, rating_category: nil) }
    ...
    context 'when the entry has been rated with a rating category' do
      ...
      let(:entry) { build(:ai_overall_comment, rating_category:) }
    ...
    context 'when the entry has been rated with a comment' do
      let(:rating_category) { create(:ai_suggestion_rating_category) }
      let(:entry) { build(:ai_overall_comment, rating_comment: 'blah') }
    ...
    context 'when the entry is internal' do
      ...
      let(:entry) { build(:ai_overall_comment, grading_suggestion_input:) }
  ...
  describe 'callbacks' do
    it 'sets the language_code from the language_code of a specified program' do
      ...
      entry = create(:ai_overall_comment, program:)
    ...
    context 'when the entry has not been reviewed' do
      it 'does not set reviewed_at to the current time' do
        entry = create(:ai_overall_comment, reviewed_by: nil)
    ...
    context 'when the entry has not been rated' do
      it 'does not set rated_at to the current time' do
        entry = create(:ai_overall_comment, rated_by: nil)
  ...
  describe '#overall_comment' do
    it 'returns the value of the overall_comment key of the overall comment' do
      feedback = { overall_comment: 'good job', explanation: 'you did it' }
      entry = build(:ai_overall_comment, evaluation_text: feedback)

      expect(entry.overall_comment).to eq('good job')
    end
  end

  describe '#explanation' do
    it 'returns the value of the explanation key of the overall comment' do
      feedback = { overall_comment: 'good job', explanation: 'you did it' }
      entry = build(:ai_overall_comment, evaluation_text: feedback)

      expect(entry.explanation).to eq('you did it')
    end
  end
end
