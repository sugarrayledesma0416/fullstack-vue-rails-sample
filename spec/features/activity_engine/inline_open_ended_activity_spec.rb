feature 'Inline Open-Ended activity', js: true, chrome: true, new_gb_sync: true do
  ...
  let(:activity) { create_inline_open_ended_activity(program) }

  let(:activity_data) do
    ActivityTest::ActivityData::OpenEnded.new(activity, media_items).tap do |act|
  ...
  context 'as a student' do
      ...
      ignoring_angular do
        ...
        for_preview_page(activity_data) do
          # Add an help request on the direction line and on a question
          {
            ...
            from_open_ended_question(1) => question_help_request_comment
            ...
            comments.each do |comment|
              validate_adding_instructor_help_request(
                ...
                helpable: [
                  ...
                  from_open_ended_question(1),
                  from_open_ended_question(2),
                  from_open_ended_question(3)
                ],
                non_helpable: []
              )
            end
          ...
          # Answer correcty question 2 and incorrectly questions 1 and 3.
          from_open_ended_question(1).choose_answer('some answer for question 1')
          from_open_ended_question(2).choose_answer('some answer for question 2')
          from_open_ended_question(3).choose_answer('some answer for question 3')
        ...
        for_accept_page(activity_data) do
          ...
          (1..3).each do |qnum|
            expect(from_open_ended_question(qnum)).not_to be_helpable
            expect(from_open_ended_question(qnum).answer).not_to be_helpable
          end
        end

        attempt = Attempt.active_attempt(student, section, activity)
        # Grade question 1 as correct
        ...
        # Grade question 2 as partially correct
        ...
        for_complete_page(activity_data) do
          ...
          # Review request
          with_element(from_open_ended_question(2).answer) do |item|
            comments = [
            ...
            comments.each do |comment|
              validate_adding_instructor_review_request(
                ...
                helpable: [
                  ...
                  from_open_ended_question(2).answer,
                  # Pending question is not helpable
                  from_open_ended_question(3).answer
                ],
                non_helpable: [
                  from_direction_line,
                  from_open_ended_question(1),
                  ...
                  from_open_ended_question(1).answer
                ]
              )
            end
    ...
    scenario 'I can use the accent bar to insert special characters' do
      ...
      for_preview_page(activity_data) do
        ...
        from_open_ended_question(1).choose_answer(str)
        ...
        expect(from_open_ended_question(1).answer.text).to eq(str)
      end
    end
    ...
    scenario 'my answers are sanitized' do
      ...
      for_preview_page(activity_data) do
        # Try harmfull answer
        from_open_ended_question(1).choose_answer(harmful_answer)
        # And save
      ...
      for_decide_page(activity_data) do
        ...
        expect(from_open_ended_question(1).answer.text).to eq(sanitized_answer)
        # Try harmfull answer
        from_open_ended_question(1).choose_answer(harmful_answer)
        # And submit
        accept_alert do
    ...
    scenario 'I can do an open ended activity' do
      question_1_answer = ''
      question_2_answer = ''
      ...
      for_preview_page(activity_data) do
        ...
        (1..4).each do |number|
          from_open_ended_question(number) do |question|
            expect(question).to be_marked(:blank)
            expect(question.answer.text).to eq('')
          end
        end

        question_1_answer = 'some random answer for question 1'
        question_2_answer = 'some random answer for question 2'

        from_open_ended_question(1).choose_answer(question_1_answer)
        from_open_ended_question(2).choose_answer(question_2_answer)
      ...
      for_decide_page(activity_data) do
        ...
        [question_1_answer, question_2_answer, '', ''].each.with_index(1) do |answer, number|
          from_open_ended_question(number) do |question|
            expect(question).to be_marked(:changed)
            expect(question.answer.text).to eq(answer)
          end
        end
      ...
      for_retry_page(activity_data) do
        ...
        # Every question should be marked as pending
        [question_1_answer, question_2_answer, '', ''].each.with_index(1).each do |answer, number|
          from_open_ended_question(number) do |question|
            expect(question).to be_marked(:pending)
            expect(question.answer.text).to eq(answer)
          end
        ...
        question_2_answer = 'A different answer for question 2'
        from_open_ended_question(2).choose_answer(question_2_answer)
      ...
      for_retry_page(activity_data) do
        ...
        from_open_ended_question(1) do |question|
          expect(question).to be_marked(:pending)
          expect(question.answer.text).to eq(question_1_answer)
        end

        from_open_ended_question(2) do |question|
          expect(question).to be_marked(:changed)
          expect(question.answer.text).to eq(question_2_answer)
        end
      ...
      for_accept_page(activity_data) do
        ...
        from_open_ended_question(2) do |question|
          ...
          1.upto(activity_data.question(2).sample_answers.size) do |i|
            expect(
              question.sample_answers.answer(i).text
            ).to eq(activity_data.question(2).sample_answers[i - 1])
          end
        end
