...
feature 'Drop down activity', js: true, chrome: true, new_gb_sync: true do
  ...
  let(:help_request_comment) { 'some request help comment' }
  let(:help_request_comment_2) { 'another request help comment' }
  let(:review_request_comment) { 'some request review comment' }
  let(:review_request_comment_2) { 'another request review comment' }
      ...
      ignoring_angular do
        ...
        for_preview_page(activity_data) do
          # Add help requests on the direction line and on a question
          {
            ...
            from_drop_down_question(1) => question_help_request_comment
            ...
            comments.each do |comment|
              question = from_drop_down_question(1)
              validate_adding_instructor_help_request(
                item: item,
                comment: comment,
                helpable: [
                  from_direction_line,
                  # whole question is helpable
                  question
                ],
                non_helpable: [
                  # answers are not helpable
                  question.drop_down_menu(1),
                  question.drop_down_menu(2),
                  question.drop_down_menu(3),
                  question.drop_down_menu(4)
                ]
              )
            end
            ...
            # Answer correctly menu 1 and incorrectly menu 2 and menu 3
            # and don't answer other menus
            menu_1_option = activity_data.question(1).menu(1).correct_option
            menu_2_option = activity_data.question(1).menu(2).incorrect_options[0]
            menu_3_option = activity_data.question(1).menu(3).incorrect_options[0]

            [menu_1_option, menu_2_option, menu_3_option].each.with_index(1) do |option, menu|
              from_drop_down_question(1).drop_down_menu(menu).select_option(option.number)
            end
          end
        ...
        for_submit_page(activity_data) do
          ...
          click_button_expect_alert(:accept, 'You have chosen to accept your grade of 25.0%')
        end
        ...
        for_accept_page(activity_data) do
          ...
          # Review request
          with_element(from_drop_down_question(1).drop_down_menu(2)) do |item|
            ...
            comments.each do |comment|
              ...
              validate_adding_instructor_review_request(
                ...
                non_helpable: [
                  from_direction_line,
                  # whole question is not helpable
                  question,
    ...
    scenario 'I can do a drop down activity' do
      ...
      for_preview_page(activity_data) do
        ...
        from_drop_down_question(1) do |question|
          ...
          question.drop_down_menus.each do |menu|
          ...
          question.drop_down_menu(1).select_option(menu_1_option.number)
          question.drop_down_menu(2).select_option(menu_2_option.number)
          question.drop_down_menu(3).select_option(menu_3_option.number)
        end
      ...
      for_decide_page(activity_data) do
        ...
        from_drop_down_question(1) do |question|
          ...
          [menu_1_option, menu_2_option, menu_3_option].each.with_index(1) do |option, number|
            with_element(question.drop_down_menu(number)) do |menu|
              expect(menu).to be_marked(:changed)
              expect(menu.option(option.number)).to be_selected
            end
          end

          with_element(question.drop_down_menu(4)) do |menu|
            expect(menu).to be_marked(:changed)
            expect(menu.options).to all_be_unselected
          end
        end
      ...
      for_submit_page(activity_data) do
        ...
        from_drop_down_question(1) do |question|
          ...
          with_element(question.drop_down_menu(1)) do |menu|
            ...
            expect(menu.text).to include(menu_1_option.text)
          end

          [menu_2_option, menu_3_option].each.with_index(2) do |option, number|
            with_element(question.drop_down_menu(number)) do |menu|
              expect(menu).to be_marked(:incorrect)
              expect(menu.text).to include(option.text)
            end
          end
      ...
      for_retry_page(activity_data) do
        ...
        from_drop_down_question(1) do |question|
          ...
          # Correct menu 2
          menu_2_option = activity_data.question(1).menu(2).correct_option
          question.drop_down_menu(2).select_option(menu_2_option.number)
        end
      ...
      for_retry_page(activity_data) do
        ...
        from_drop_down_question(1) do |question|
          ...
          with_element(question.drop_down_menu(2)) do |menu|
            expect(menu).to be_marked(:changed)
            expect(menu.option(menu_2_option.number)).to be_selected
          end

          with_element(question.drop_down_menu(3)) do |menu|
            expect(menu).to be_marked(:incorrect)
            expect(menu.option(menu_3_option.number)).to be_selected
          end
      ...
      for_submit_page(activity_data) do
        ...
        from_drop_down_question(1) do |question|
          ...
          with_element(question.drop_down_menu(2)) do |menu|
            expect(menu).to be_marked(:correct)
            expect(menu.text).to include(menu_2_option.text)
          end

          with_element(question.drop_down_menu(3)) do |menu|
            expect(menu).to be_marked(:incorrect)
            expect(menu.text).to include(menu_3_option.text)
          end
