feature 'model Reference', chrome: true, js: true do
    ...
    let(:activity) do
      create(:json_activity_with_model, title: 'Mid-Unit Assessment', lesson:)
    end
    let(:category) { create(:category, course:, penalty_percent: 0) }

    def validate_model_reference
      step 'I can see model reference header' do
        expect(page).to have_selector(
          '.reference_model > h3',
          text: 'Model reference header'
        )
      end

      step 'I can see model reference body' do
        within('.reference_model_body') do
          expect(page).to have_selector('p', text: 'Model reference body')
        end
      end
    end

    scenario 'As a student, I can view the assessment with model reference ' \
          'in preview mode' do
