RSpec.describe Instructor::CreatedActivity::NewActivityLinkAssessmentPresenter do
  include ActionView::TestCase::Behavior
  subject(:presenter) { described_class.new(view, instructor_presenter) }

  let(:instructor_presenter) { instance_double(InstructorAssessmentTocPresenter) }
  let(:program_id) { 123  }
  ...
  before do
    ...
    allow(instructor_presenter).to receive(:current_strand).and_return(current_topic)
  end

  describe '#menubar_classes' do
    it 'return expected classes' do
      expected_classes = %w[c-menubar js-nav-system c-menu--admin]
  ...
  describe '#menu_subitem_link_classes' do
    context 'when the activity type is exam' do
      it 'return expected classes' do
        expected_classes = %w[assessment_link js-nav-system__subnav__link]
        expect(presenter.menu_subitem_link_classes(:exam).split)
          .to match_array(expected_classes)
      end
    end

    context 'when the activity type is not exam' do
      it 'return expected classes' do
        expect(presenter.menu_subitem_link_classes(:other_than_exam))
          .to eq('js-nav-system__subnav__link')
      end
    end
  end
  ...
  describe '#new_created_activity_link' do
    ...
    before do
      allow(Rails).to receive_message_chain(
        :application, :routes, :url_helpers, instructor_path
    ...
    context 'when the activity type is exam' do
      let(:instructor_path) { :instructor_new_assessment_path }
      let(:activity_type) { :exam }
      let(:activity_params) { {} }
      ...
      it 'calls instructor_new_assessment_path properly' do
        expected_params = {
          program_id:,
        ...
        expect(Rails.application.routes.url_helpers)
          .to have_received(:instructor_new_assessment_path).with(expected_params)
      end
    ...
    context 'when the activity type is not exam' do
      let(:instructor_path) { :new_instructor_created_activity_path }
